...
上述示例需要当前 K8S 集群中有 ReplicaLimit 这个资源,因此可以作为效果理解但没办法实践.

下面实践一下这个参数化策略的效果,将 CR 资源修改为使用当前 K8S 集群中已有的 Deployment 资源.
...
如果应用在带有标签`environment=production`的 namespace,那么 Deployment 资源的副本数必须不能超过 kube-system namespace 下 coredns 这个 Deployment 的副本数.
...
The deployments "envoy" is invalid: : ValidatingAdmissionPolicy 'demo-policy.lank8s.local' with binding 'demo-binding-production.lank8s.local' denied request: failed expression: object.spec.replicas <= params.spec.replicas
```
